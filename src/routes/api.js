...
router.get('/health', async (req, res) => {
  try {
    ...
    // mode: cómo se están generando las respuestas ahora mismo
    //   'mock'          -> MOCK_LLM=true (nunca se contacta a Ollama)
    //   'ollama'        -> Ollama disponible, respuestas reales
    //   'mock-fallback' -> Ollama caído pero LLM_FALLBACK_MOCK activo (respuestas simuladas)
    //   'unavailable'   -> Ollama caído y sin fallback (las llamadas fallarán)
    let mode = 'ollama';
    if (config.llm.mock) mode = 'mock';
    else if (!ollamaConnected) mode = config.llm.fallbackToMockOnError ? 'mock-fallback' : 'unavailable';
    ...
    res.json({
      ...
      llm: {
        ...
        fallbackToMockOnError: config.llm.fallbackToMockOnError
      },
      services: {
